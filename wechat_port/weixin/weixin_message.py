...
from .weixin_api import download_media_from_cdn, CDN_BASE_URL
...
class WeixinMessage(ChatMessage):
    ...
    def _download_media(self, item: dict, media_type: int, cdn_base_url: str) -> str:
        ...
        if not encrypt_param or not aes_key:
            logger.warning(f"[Weixin] 媒体下载缺少CDN参数 (type={media_type})")
            return ""
        ...
        try:
            ...
            logger.info(f"[Weixin] 媒体已下载: {save_path}")
            return save_path
